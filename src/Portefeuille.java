	...
	public boolean transfertDevise (Portefeuille destination, double montantJetons){
		...
		**/
		...
		if ( this.montant >= destination.montant && 
			 this.monnaie.getNom().equals( destination.monnaie.getNom() ) &&
			 this.montant - montantJetons > 0 )
		{
			destination.montant = destination.getMontant() + montantJetons * this.monnaie.getValeurDeJeton();
			this.montant -= montantJetons * this.monnaie.getValeurDeJeton();
			return true;
		}
